...
from score import score_circle_amount, score_circle_values, score_circle_opponent_min
...
if __name__ == '__main__':
    ...
    DEPTH_J2 = 2 # PROFUNDIDAD JUGADOR 2
    ...
    JUGADOR_2 = Player("MINIMAX", GREEN, 1, DEPTH_J2, eval_func=score_circle_opponent_min)
    ...
    for i in range(EXPERIMENTS):
        ...
        while True:
            ...
            if JUGADOR.tipo == "HUMANO":
                ...
                x, y = result[1]
                print(f"Jugador {JUGADOR.id + 1} eligió la jugada {x,y}")
            end_time = time.time()
            jugadas += 1
            ...
            if is_winner(circles, JUGADORES):
                ...
                print(f"\nJugador {JUGADOR.id + 1} es el ganador")
                ...
                if JUGADOR.id == 0:
                    ...
                    win_J2 += 1
                prom_tiempos_J1.append(round(sum(tiempos_J1)/len(tiempos_J1),3))
                prom_tiempos_J2.append(round(sum(tiempos_J2)/len(tiempos_J2),3))

                break
    if (EXPERIMENTS > 1):
        print("\n\n\n **** Resultados finales: ****")
        print(f"Jugador 1 gano {win_J1} veces")
        print(f"Jugador 2 gano {win_J2} veces")
        print(f"Jugador 1 tardo en promedio {round(sum(prom_tiempos_J1)/len(prom_tiempos_J1),3)} por jugada.")
        print(f"Jugador 2 tardo en promedio {round(sum(prom_tiempos_J2)/len(prom_tiempos_J2),3)} por jugada.")
