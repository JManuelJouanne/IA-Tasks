...
class Map:
    ...
    def set_connectivity(self, n):
        ...
        if n == 4:
            ...
            rotations = [(1/2)*np.pi, (1)*np.pi, (3/2)*np.pi, (2)*np.pi]
            ...
            for k in range(len(rotations)):
                neighbor = self.generate_primitive_neighbor(initialPose,
                                                            numberOldPose, initialMove, [initialMove], rotations, k, cost)
            ...
            rotations = [(1/4)*np.pi, (3/4)*np.pi, (5/4)*np.pi, (7/4)*np.pi]
            numberOldPose = 0  # 0 grados
    ...
    def octile(x1, y1, x2, y2):
        ...
        min_d = min(dx, dy)
        max_d = max(dx, dy)
        return max_d - (math.sqrt(2) - 1)*min_d
        ...
        for y in range(0, self.size_y):
            for x in range(0, self.size_x):
                if x == self.init_x and y == self.init_y:
                    ...
                    print('G', end='')
                elif self.ice[x][y] is True:
                    print(colored('*', 'cyan'), end='')
                elif self.sand[x][y] is True:
                    print(colored('*', 'yellow'), end='')
                elif (x, y) in trace:
                    print(colored('*', 'green'), end='')
                elif (x, y) in generated_positions:
                    print(colored('X', 'red'), end='')
                elif self.map[x][y] is True:
                    print('.', end='')
                elif self.map[x][y] is False:
